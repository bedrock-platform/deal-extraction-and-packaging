"""
Incremental Exporter for Package Creation (Stage 2)

Saves packages immediately after creation and appends to Google Sheets.
"""
import json
import logging
...
class PackageIncrementalExporter:
    """
    Exports packages incrementally (as they're created) to files and Google Sheets.
    ...
    def __init__(
        ...
        # File paths
        self.jsonl_path = output_dir / f"packages_{timestamp}.jsonl"
        self.json_path = output_dir / f"packages_{timestamp}.json"
    ...
    def _init_google_sheets(self) -> None:
        ...
        try:
            ...
            if spreadsheet:
                # Get or create "Packages Enriched 1" worksheet
                try:
                    self.worksheet = spreadsheet.worksheet("Packages Enriched 1")
                    ...
                    if len(existing_data) > 1:  # Has header + data
                        ...
                        logger.info(f"Found existing 'Packages Enriched 1' worksheet with {len(existing_data) - 1} rows")
                        ...
                        logger.info("Found 'Packages Enriched 1' worksheet header, will append packages")
                    else:
                        # Empty worksheet
                        self.sheets_header_written = False
                        self.sheets_next_row = 2
                        logger.info("Found empty 'Packages Enriched 1' worksheet")
                    
                    # Expand worksheet columns if needed
                    try:
                        current_cols = self.worksheet.col_count
                        cols_needed = max(50, current_cols, 40)  # Packages have many fields
                    ...
                    self.worksheet = spreadsheet.add_worksheet(title="Packages Enriched 1", rows=2000, cols=50)
                    logger.info("Created new 'Packages Enriched 1' worksheet")
            else:
                self.worksheet = None
    ...
    def export_package(self, package: Dict[str, Any]) -> None:
        """
        Export a single package immediately.
        
        Args:
            package: Package dictionary to export
        """
        # Append to JSON Lines file
        self._append_jsonl(package)
        
        # Append to JSON array file
        self._append_json(package)
        
        # Append to Google Sheets
        if self.worksheet:
            self._append_to_sheets(package)
    ...
    def _append_to_sheets(self, package: Dict[str, Any]) -> None:
        """Append package row to Google Sheets."""
        ...
        try:
            ...
            logger.debug(f"Appended package to Google Sheets (row {self.sheets_next_row}, {num_cols} cols)")
